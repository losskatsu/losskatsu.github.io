...
모든 인풋/아웃풋 위치에 대해 은닉 표현(hidden representation)을 병렬처리로 계산합니다. 
그리고 이러한 모델들은 엄청난 양의 연산을 요구합니다. 
왜냐하면 임의의 두 인풋 또는 아웃풋 위치 간의 신호를 연결하는데 필요한 연산 횟수가 
위치간 거리에 따라 증가하기 때문입니다. 
ConvS2S는 선형적으로, ByteNet의 경우 로그 함수 형태로 증가합니다. 
