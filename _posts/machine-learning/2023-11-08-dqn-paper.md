...
1. 우선 초기화 replay memory 초기화가 필요한데, replay memory $D$를 공간 $N$을 가지도록 초기화 합니다.  
2. action-value function $Q$를 랜덤 가중치를 줘서 초기화 합니다.  
3. for episode = (1, $M$), 에피소드 1부터 $M$까지 반복문을 수행합니다.  
   3.1. 시퀀스 $s_1 = \\{x_1\\}$과 preprocessed sequenced $\phi_1 = \phi(s_1)$을 초기화 합니다.  
   3.2. for $t=1,T$, 타임스텝 $t$에 대해 반복문을 수행합니다.   
       3.2.1. $\epsilon$ 확률로 액션 $a_t$를 선택합니다.   
       3.2.2. 그 밖의 $a_t = max_a Q^{\*}(\phi(s_t), a ; \theta)$를 선택합니다.   
       3.2.3. 에뮬레이터 내부에서 액션 $a_t$를 실행하고 보상 $r_t$와 이미지 $x_{t+1}$을 관측합니다.   
       3.2.4. 그리고나서 $s_{t+1} = s_t, a_t, x_{t+1}$과 $\phi_{t+1} = \phi(s_{t+1})$을 설정합니다.   
       3.2.5. transition $(\phi_t, a_t, r_t, \phi_{t+1})$을 $D$에 저장합니다.    
       3.2.6. $D$로부터의 트랜지션 $(\phi_j, a_j, r_j, \phi_{j+1})$의 미니배치를 샘플링합니다.    
       3.2.7. 타겟 $y_j$를 셋팅하는데, $terminal \phi_{j+1}$에 대해서는 $y_j=r_j$라고 설정하고, $non terminal \phi_{j+1}$에 대해서는 $y_j=r_j + \gamma max_{a^{\prime}}Q(\phi_{j+1}, a^{\prime}; \theta)$라고 설정합니다.    
       3.2.8. 앞서 언급한 방정식 3에 따라 $(y_j - Q(\phi_j, a_j ; \theta))^2$에 대한 그레디언트 디센트 단계를 수행합니다.  
   3.3. 반복문을 종료합니다.   
4. 반복문을 종료합니다.  
