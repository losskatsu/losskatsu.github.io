...
# [우분투]pyenv 를 이용해 파이썬 가상환경 구축하기(최근 업데이트: 2022.09.26) 
...
저는 현재 가장 최신 파이썬 버전인 3.10.4를 설치하겠습니다.
...
(랙 걸린거 아님, 조금 기다려야함)
Installed Python-3.10.4 to /home/ubuntu/.pyenv/versions/3.10.4
```
...
<br />
저는 3.10.4  버전의 가상환경이므로 py3_10_4 라는 가상환경이름으로 짓겠습니다.
...
텐서플로(tensorflow) 설치
```bash
(py3_8_5)$ pip install tensorflow==2.6.0 
```
...
파이토치 설치(CPU) 버전
```bash
(py3_8_5)$ pip install torch torchvision torchaudio
```
